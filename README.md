...
### 功能特性
- **🎨 n8n 风格设计**：现代化暗色主题界面，类似 n8n 的专业外观
- **📋 任务管理**：创建、编辑、启动、停止、删除任务
- **🔗 可视化工作流编辑器**：拖拽式 DAG 编辑器，直观创建和编辑工作流
  - 拖拽添加节点
  - 可视化连接节点形成 DAG
  - 节点属性配置
  - 实时预览工作流结构
...
npm run dev
```
访问 `http://localhost:3000` 进行开发，支持热更新。
...
npm run build
```
构建产物会自动输出到 `admin/src/main/resources/static` 目录。

### 使用说明
1. 启动 Admin 服务后，浏览器访问 `http://localhost:8080`
2. **任务管理**：在任务管理页面可以创建新任务，配置 Cron 表达式、处理器等
3. **工作流管理**：点击"新建工作流"进入可视化编辑器
   - 从左侧节点面板拖拽节点到画布
   - 点击节点间的连接点创建边
   - 点击节点配置属性（任务ID、颜色等）
   - 保存后可启动工作流
4. **执行实例**：查看任务执行情况和日志
5. **实时监控**：连接 WebSocket 监听任务事件

## 🔖 TODO
1. 支持秒级任务调度
2. api 及 Hazelcast 节点安全性
3. ~~新增 web 页面~~ ✅ 已完成（Vue 3 + n8n 风格界面 + 可视化工作流编辑器）
